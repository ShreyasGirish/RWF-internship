...
const createPerson = async () => {
    ...
    try {
      ...
      alert('Failed to create person folder. Is backend running?');
    }
  };
...
const registerFace = async () => {
    ...
    try {
      ...
      alert('Failed to start registration. Is backend running?');
    }
  };
...
const stopStream = async () => {
    try {
      const res = await fetch(`${BASE_URL}/api/stop-stream`, { method: "POST" });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      setStreaming(false);
    } catch (error) {
      console.error('Stop stream failed:', error);
      alert('Failed to stop stream. Server may be busy.');
      setStreaming(false);
    }
  };
...
const startCamera = async () => {
    try {
      ...
      alert('Failed to start attendance camera. Is backend running?');
    }
  };
  ...
  const stopCamera = async () => {
    try {
      ...
      alert('Failed to stop attendance camera.');
    }
  };
